...
@app.route('/venues')
def venues():
    venue_groups = Venue.query.with_entities(func.count(
        Venue.id), Venue.city, Venue.state).group_by(Venue.city, Venue.state).all()
    data = []
    for obj in venue_groups:
        venues = Venue.query.filter_by(city=obj[1], state=obj[2])
        group = {"city": obj[1], "state": obj[2], "venues": venues}
        data.append(group)
...
def show_venue(venue_id):
    ...
    upcoming_shows = []
    for show in shows:
      obj = {"artist_id": show.artist.id, "artist_name": show.artist.name, "artist_image_link": show.artist.image_link, "start_time": show.start_time}
      if show.start_time > current_time:
        upcoming_shows.append(obj)
      else:
        past_shows.append(obj)
    data = {"id": venue.id, "name": venue.name, "genres": venue.genres, "address": venue.address, "city": venue.city, "state": venue.state, "phone": venue.phone, "website": venue.website_link, "facebook_link": venue.facebook_link, "seeking_talent": venue.seeking_talent, "seeking_description": venue.seeking_description, "image_link": venue.image_link, "past_shows": past_shows, "upcoming_shows": upcoming_shows, "past_shows_count": len(past_shows), "upcoming_shows_count": len(upcoming_shows)}
...
def shows():
    ...
    data = []
    for show in shows:
        obj = {"venue_id": show.venue.id, "venue_name": show.venue.name, "artist_id": show.artist.id,
